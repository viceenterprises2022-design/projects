...
# Pre-configured production key from your validator
...
def fetch_inbound_senders(address):
    """Fetches unique sender addresses from inbound transfers by querying all and filtering in Python."""
    senders = set()
    headers = get_headers()
    
    # 1. Fetch TRX Activity (direction=0 for ALL to avoid mapping errors)
    ...
    params = {
        ...
        "reverse": "true", 
        "limit": 50,
        "db_version": 1  # Required for fresh data
    }
    try:
        res = requests.get(trx_url, params=params, headers=headers, timeout=15)
        data = res.json().get('data', [])
        for tx in data:
            # Manually filter for Inbound (Recipient is our target)
...
def main():
    ...
    with console.status(f"[bold green]Scanning inbound activity for {target_wallet}...[/]"):
        senders = fetch_inbound_senders(target_wallet)
        
        if not senders:
            console.print(f"\n[yellow]No inbound transfers found for {target_wallet}.[/]")
            console.print("[dim]Note: This address may have only OUTBOUND activity or other token types.[/]\n")
            return
        ...
        for sender in senders:
            if not sender or sender == target_wallet: continue
