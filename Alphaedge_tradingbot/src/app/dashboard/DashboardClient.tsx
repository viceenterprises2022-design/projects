      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerRight}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginRight: '16px' }}>
            {user?.image && (
              <img src={user.image} alt="Profile" style={{ width: '24px', height: '24px', borderRadius: '50%' }} />
            )}
            <span style={{ fontSize: '12px', color: '#94a3b8' }}>{user?.name || user?.email}</span>
            <button 
              onClick={() => handleSignOut()}
              style={{ backgroundColor: 'transparent', border: '1px solid #1f2b45', color: '#f1f5f9', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}>
              Sign out
            </button>
          </div>
