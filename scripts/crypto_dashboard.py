...
Uses Order Book Depth to predict liquidation zones with Buy/Sell breakdown.
"""
...
def fetch_deribit_quotes(currency):
    ...
    try:
        ...
        if currency == "SOL":
            # Filter for SOL_USDC instruments
...
def calculate_max_pain(options_data):
    ...
    for opt in options_data:
        ...
        try:
            # For SOL_USDC-..., parts[2] is the strike
            # For BTC-..., parts[2] is the strike
...
def generate_liquidation_map(depth_data, symbol):
    """
    Groups order book depth into price bins with Buy/Sell breakdown.
    Returns: list of (price_bin, buy_vol, sell_vol, total_vol)
    """
    if not depth_data: return []
    
    bin_size = 100 if symbol == "BTC" else (10 if symbol == "ETH" else 1)
    # {price_bin: {"buy": vol, "sell": vol}}
    ...
    # Flatten and sort by total volume
    flattened = []
    for p, v in bins.items():
        total = v["buy"] + v["sell"]
        flattened.append((p, v["buy"], v["sell"], total))
        
    sorted_map = sorted(flattened, key=lambda x: x[3], reverse=True)
...
def make_liquidation_map_table(liq_map, perp_oi):
    ...
    table.add_column("BUY LIQ", justify="right", style="green")
    table.add_column("SELL LIQ", justify="right", style="red")
    table.add_column("DENSITY", justify="left")
    
    if not liq_map:
        table.add_row("Calculating...", "", "", "[░░░░░░░░░░░░░░░]")
        return table
        
    max_liq = max(b[3] for b in liq_map)
    for price, buy, sell, total in liq_map:
        bar_len = int((total / max_liq) * 15)
        
        # Color bar based on dominant side
        char = "█"
        color = "[green]" if buy > sell else "[red]"
        bar = f"{color}{char * bar_len}[/]" + "░" * (15 - bar_len)
