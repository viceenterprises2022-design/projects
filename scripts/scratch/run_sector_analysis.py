...
def main():
    print("Step 1: Finding skill 'altcoin_sector_analysis'...")
    find_res = call_mcp_tool("tools/call", {
        "name": "find_skill",
        "arguments": {
            "query": "altcoin_sector_analysis"
        }
    })
    ...
    for cand in candidates:
        ...
        if "altcoin_sector_analysis" in name.lower() or "sector" in name.lower():
            target_skill = cand
            break
            
    if not target_skill:
        print("\n[FAILURE] Skill 'altcoin_sector_analysis' not found in CMC Skill Hub.")
        print("Alternatives:")
    ...
    with open("scratch/sector_analysis_output.json", "w") as f:
        json.dump(exec_res, f, indent=2)
    print("\nSaved execution results to scratch/sector_analysis_output.json")
