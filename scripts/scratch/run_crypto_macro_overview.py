...
def main():
    print("Step 1: Finding skill 'crypto_macro_overview'...")
    find_res = call_mcp_tool("tools/call", {
        "name": "find_skill",
        "arguments": {
            "query": "crypto_macro_overview"
        }
    })
    ...
    for cand in candidates:
        ...
        if "crypto_macro" in name.lower() or "macro_overview" in name.lower() or "crypto" in name.lower():
            target_skill = cand
            break
            
    if not target_skill:
        print("\n[FAILURE] Skill 'crypto_macro_overview' not found in CMC Skill Hub.")
        print("Alternatives:")
    ...
    with open("scratch/crypto_macro_overview_output.json", "w") as f:
        json.dump(exec_res, f, indent=2)
    print("\nSaved execution results to scratch/crypto_macro_overview_output.json")
