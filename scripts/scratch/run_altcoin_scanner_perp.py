...
def main():
    print("Step 1: Finding skill 'altcoin_scanner_perp'...")
    find_res = call_mcp_tool("tools/call", {
        "name": "find_skill",
        "arguments": {
            "query": "altcoin_scanner_perp"
        }
    })
    ...
    for cand in candidates:
        ...
        if "altcoin_scanner" in name.lower() or "scanner_perp" in name.lower() or "altcoin" in name.lower():
            target_skill = cand
            break
            
    if not target_skill:
        print("\n[FAILURE] Skill 'altcoin_scanner_perp' not found in CMC Skill Hub.")
        print("Alternatives:")
    ...
    with open("scratch/altcoin_scanner_perp_output.json", "w") as f:
        json.dump(exec_res, f, indent=2)
    print("\nSaved execution results to scratch/altcoin_scanner_perp_output.json")
