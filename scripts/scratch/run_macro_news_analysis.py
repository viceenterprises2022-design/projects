...
def main():
    print("Step 1: Finding skill 'macro_news_aggregator'...")
    find_res = call_mcp_tool("tools/call", {
        "name": "find_skill",
        "arguments": {
            "query": "macro_news_aggregator"
        }
    })
    ...
    for cand in candidates:
        ...
        if "macro_news" in name.lower() or "news" in name.lower():
            target_skill = cand
            break
            
    if not target_skill:
        print("\n[FAILURE] Skill 'macro_news_aggregator' not found in CMC Skill Hub.")
        print("Alternatives:")
    ...
    with open("scratch/macro_news_analysis_output.json", "w") as f:
        json.dump(exec_res, f, indent=2)
    print("\nSaved execution results to scratch/macro_news_analysis_output.json")
