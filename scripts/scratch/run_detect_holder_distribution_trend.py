...
def main():
    print("Step 1: Finding skill 'detect_holder_distribution_trend'...")
    find_res = call_mcp_tool("tools/call", {
        "name": "find_skill",
        "arguments": {
            "query": "detect_holder_distribution_trend"
        }
    })
    ...
    for cand in candidates:
        ...
        if "holder" in name.lower() or "distribution" in name.lower() or "trend" in name.lower():
            target_skill = cand
            break
            
    if not target_skill:
        print("\n[FAILURE] Skill 'detect_holder_distribution_trend' not found in CMC Skill Hub.")
        print("Alternatives:")
    ...
    with open("scratch/detect_holder_distribution_trend_output.json", "w") as f:
        json.dump(exec_res, f, indent=2)
    print("\nSaved execution results to scratch/detect_holder_distribution_trend_output.json")
