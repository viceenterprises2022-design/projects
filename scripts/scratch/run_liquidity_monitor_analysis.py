...
def main():
    print("Step 1: Finding skill 'macro_liquidity_monitor'...")
    find_res = call_mcp_tool("tools/call", {
        "name": "find_skill",
        "arguments": {
            "query": "macro_liquidity_monitor"
        }
    })
    ...
    for cand in candidates:
        ...
        if "liquidity" in name.lower():
            target_skill = cand
            break
            
    if not target_skill:
        print("\n[FAILURE] Skill 'macro_liquidity_monitor' not found in CMC Skill Hub.")
        print("Alternatives:")
    ...
    with open("scratch/liquidity_monitor_analysis_output.json", "w") as f:
        json.dump(exec_res, f, indent=2)
    print("\nSaved execution results to scratch/liquidity_monitor_analysis_output.json")
