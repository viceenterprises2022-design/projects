...
def upsert_position(symbol: str, side: str, size: float, entry_price: float, leverage: float, margin: float, unrealized_pnl: float):
    now_str = datetime.datetime.utcnow().isoformat()
    with get_conn() as conn:
        conn.execute("""
            INSERT INTO paper_positions (symbol, side, size, entry_price, leverage, margin, unrealized_pnl, timestamp)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ...
        """, (symbol, side, size, entry_price, leverage, margin, unrealized_pnl, now_str))
        conn.commit()
...
def add_trade(symbol: str, side: str, type_str: str, price: float, size: float, pnl: float):
    now_str = datetime.datetime.utcnow().isoformat()
    with get_conn() as conn:
        conn.execute("""
            INSERT INTO paper_trades (symbol, side, type, price, size, pnl, timestamp)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (symbol, side, type_str, price, size, pnl, now_str))
        conn.commit()
